...
import { controlPillIdle, controlPillShell } from './control-pill'
...
function SelectTrigger({
  ...
  size?: 'sm' | 'default'
  /** Replaces the default chevron in the trigger's trailing icon slot. */
  ...
  return (
    <SelectPrimitive.Trigger
      ...
      data-size={size}
      // Composed from the shared control pill (ui/control-pill) — the SAME
      // shell the filter popovers use, so dropdowns and filters stay visually
      // identical by construction. `size='sm'` keeps the pill's native h-8 /
      // 12px-semibold; the default size relaxes to h-9 / text-sm so it lines
      // up with Input in forms (twMerge resolves those overrides).
      className={cn(
        controlPillShell,
        controlPillIdle,
